...
void SelfStarting()
{
    ...
    if (Win::GetExePathName(ExePathName)) {
        if (Tools.BaseConfigItem[Tx("注册表开机自启动")] != Tx("是")) {
...
void LoadBaseConfig(bool _bReLoad)
{
    ...
    if (Tools.FileSystem.CreateDirectorys()) {
        ...
        if (!Tools.ConfigFile.Init(Format(Tx("%"), Tools.ExeConfigFilePath))) {
            ...
            BaseConfig.push_back(Tx("默认的屏幕分辨率宽=1920"));
            BaseConfig.push_back(Tx("默认的屏幕分辨率高=1080"));
...
void UpdateConfig()
{
    ...
    lgc(Tip, Format(Tx("  默认的屏幕分辨率宽: %"), Tools.BaseConfigItem[Tx("默认的屏幕分辨率宽")]));
    lgc(Tip, Format(Tx("  默认的屏幕分辨率高: %"), Tools.BaseConfigItem[Tx("默认的屏幕分辨率高")]));
...
void LoadToolsMenu(HMENU Menu)
{
    ...
    lgc(Tip, Format(Tx("Tools.ID_ToolsConfig: [%]"), Tools.ID_ToolsConfig));
    Tools.ID_Help = WindowHost::GetHMENU();
    lgc(Tip, Format(Tx("Tools.ID_Help: [%]"), Tools.ID_Help));
    Tools.ID_Quit = WindowHost::GetHMENU();
    lgc(Tip, Format(Tx("Tools.ID_Quit: [%]"), Tools.ID_Quit));
...
void SelectMenu(int MenuItemID)
{
    ...
    {
        lgc(Format(Tx("菜单项ID: ID_ToolsConfig Path: [%]"), Tools.ConfigFile.GetConfigFilePath()));
...
\t模式=打开文件 (打开文件/打开文件夹/管理员运行)\n\t菜单按键=是 (是/否: 托盘菜单中添加/程序启动时运行)"));
    ...
    else if (MenuItemID == Tools.ID_SetScreenResolution)
    {
        ...
        if (Tools.SetScreenResolution)
        {
            //SetScreenResolution
            SetDisplaySize(
                std::stoi(Tools.BaseConfigItem.find(Tx("默认的屏幕分辨率宽"))->second),
                std::stoi(Tools.BaseConfigItem.find(Tx("默认的屏幕分辨率高"))->second)
            );
            Tools.SetScreenResolution = false;

            lgc(Tip, Format(Tx("屏幕分辨率修改成功:\n[%] x [%]"), 
                Tools.BaseConfigItem.find(Tx("默认的屏幕分辨率宽"))->second, Tools.BaseConfigItem.find(Tx("默认的屏幕分辨率高"))->second));
        }
        else
        {
            //SetScreenResolution
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_HOTKEY:
    {
        ...
        //        //SetScreenResolution
        //        SetDisplaySize(
        //            std::stoi(Tools.BaseConfigItem[Tx("默认的屏幕分辨率宽")]),
        //            std::stoi(Tools.BaseConfigItem[Tx("默认的屏幕分辨率高")])
        //        );
        //        Tools.SetScreenResolution = false;

        //        lgc(Format(Tx("屏幕分辨率修改成功:\n[%] x [%]"), Tools.BaseConfigItem[Tx("默认的屏幕分辨率宽")], Tools.BaseConfigItem[Tx("默认的屏幕分辨率高")]));
        //    }
        //    else
        //    {
        //        //SetScreenResolution
